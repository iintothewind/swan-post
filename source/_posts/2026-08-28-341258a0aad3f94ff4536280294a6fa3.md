...
```mermaid
flowchart TD
    accTitle: Agent-native stack layers
    accDescr: Skill Layer holds db.connect and db.query and db.explain and db.workflow and db.export, which flows through the Transport layer of HTTPS or SSE or WebSocket into a stateless ephemeral Execution Sandbox. The sandbox is DuckDB plus an SSH tunnel to a readonly DB user plus resource quotas on time and rows and bytes plus an audit log recording intent, plan, SQL and result
...
```mermaid
sequenceDiagram
    accTitle: Self-driving installation loop
    accDescr: The agent fetches the db-access skill manifest from the registry, receives the install prompt with tool schemas and security policy, parses it and plans the installation, provisions DuckDB and SSH config and readonly credentials in the sandbox, validates connectivity with smoke tests, registers its tools, and finally tells the user it is ready. No human ops step in between
    participant Agent
