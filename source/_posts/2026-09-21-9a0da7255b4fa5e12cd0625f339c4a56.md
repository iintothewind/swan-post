...
社区仓库 **`dolf3131/qwen3.8-flash-next-dgx-spark`**：在单台 **DGX Spark（121 GiB 统一内存）** 上，用 **NVFP4** 量化主权重，把 **51B 的 Engram 表页到 SSD swap**——装不进内存的表，靠 NVMe 跑起来。

仓库 **`Felliks/qwen38-flash-next-one-dgx-spark`**：SGLang + MTP + 原生 262K context + **NVMe-backed PLE cache**，可复现部署。
