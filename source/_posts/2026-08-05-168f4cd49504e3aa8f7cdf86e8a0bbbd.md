...
```mermaid
flowchart TB
    accTitle: 官方 Pi 与 omp 的上下文组合方式对比
    accDescr: 上半部分是官方 Pi 的 entry 到消息投影链路，SessionContext 经 defaultContextEntryTransform 找到最近的 compaction entry 替换历史，再投影为 AgentMessage 列表交给 provider。下半部分是 omp 的三层叠加，StablePrefix 与 AppendOnlyLog 与 advisor 上下文共同汇入发给 provider 的上下文
...
```mermaid
sequenceDiagram
    accTitle: 稳定前缀与追加日志下的缓存经济学
    accDescr: 三个相位。常态路径下 omp 只发送字节相同的前缀加新增 delta，provider 命中前缀缓存，只计 delta 与 cacheRead。若 prune 就地改写深处消息，provider 要从改写点重算整段后缀 KV，产生 cacheWrite 溢价，所以尽量避开。idle 超过 90 分钟后缓存已冷，此时深度清理 superseded 与 useless 结果重写免费
    autonumber
    actor U as 用户
...
```mermaid
flowchart TD
    accTitle: omp 上下文维护的触发流程
    accDescr: 每轮 turn end 先跑 pruneStaleToolResults 做超驰淘汰与 useless 剔除，再跑 pruneToolOutputs 做年龄裁剪，然后判断是否超过阈值。未超阈值正常继续。超阈值先尝试 tryContextPromotion 换更大窗口模型，零数据损失。提升失败则按用户设定的 compaction.strategy 分派到 snapcompact 或 context-full 或 handoff 或 shake 或 off。pre-prompt 与 mid-turn 与 overflow 与 idle 与 manual 等相位都汇入同一阈值判断
