...
graph TD
    accTitle: 稳定前缀的构建流程
    accDescr: 加载阶段依次读取 SYSTEM.md 的用户与项目层级、扫描 skills 目录加载技能及其描述、读取 AGENTS.md 与 CODECOPILOT.md 等上下文文件、加载 RULES.md、把工具转换为元数据格式，五路汇入汇总节点后进入组合阶段，由 System.md 与 Project System.md 与技能描述与上下文文件与工具列表与 MCP 工具说明与规则共同组成最终系统提示词
    subgraph 加载阶段
...
```mermaid
sequenceDiagram
    accTitle: Prompt Cache 的复用示意
    accDescr: 第一次请求把一万 token 的稳定前缀与一百 token 的用户消息一起送入 Prompt Cache，缓存整个请求的计算结果。后续请求复用稳定前缀那部分缓存，只为新增的用户消息与历史对话共六百 token 付出计算
...
graph TD
    accTitle: 对话树的分支结构
    accDescr: 根节点分出三个分支。分支 1 下挂一条用户消息与一条助手消息，分支 2 是压缩节点，分支 3 下挂另一组用户与助手消息。根节点与三个分支分别用不同底色区分
    Root([根节点])
...
flowchart LR
    accTitle: 预防性压缩的两种情形
    accDescr: 上一条路径上下文十万 token 占 128K 窗口的 78%，属于安全范围直接继续发送。下一条路径上下文十一万 token 占 86%，接近临界点，触发压缩降到八万 token 后再发送请求
...
```mermaid
flowchart TD
    accTitle: 切换模型时的上下文调整流程
    accDescr: 用户切到 o1-mini 后检测到当前模型是 1M token 窗口的 GPT-4o 而目标模型是 128K 窗口的 o1-mini，于是关闭当前 provider 会话并重新计算上下文占用，发现八十万 token 远超十二万八千上限，触发紧急压缩降到十万 token，再切换模型并发送请求
...
graph TD
    accTitle: 压缩方法的降级链路
    accDescr: 上下文窗口缩小到 128K 而当前占用五十万 token 时依次尝试五种方法。remote 成功则压到十五万继续，失败转 snapcompact。snapcompact 因模型不支持视觉而失败则转 handoff 生成人工可读文档并暂停等待用户，再失败转 shake 机械省略压到十三万继续，最后转 soft 用 LLM 摘要压到十二万继续
...
```mermaid
flowchart LR
    accTitle: 极端超限时的分批压缩过程
    accDescr: 从两百万 token 出发连续压缩四次，依次压掉八十万与六十万与四十五万与两万五千 token，剩余量从一百二十万降到六十万再到十五万，最后到十二万五千，达到 128K 上限
...
sequenceDiagram
    accTitle: 上下文管理对用户透明
    accDescr: 用户提出继续刚才的研究，OMP 内部检测到上下文超限后自动压缩再继续对话，用户只可能看到短暂的正在压缩提示，得到的回复与未超限时无异
